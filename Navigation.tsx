...
export function Navigation() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-5 md:px-8 max-w-7xl">
        <div className="flex items-center gap-4">
            ...
            <div className="flex items-center bg-secondary/70 dark:bg-white/[0.04] rounded-full px-1.5 py-1 border border-border/80 dark:border-white/10">
              {links.filter((link) => link.href !== "/contact").map((link) => (
                <Link
                  ...
                  className={cn(
                    "text-[13px] font-medium transition-colors px-3 lg:px-3.5 py-2 rounded-full whitespace-nowrap",
                    location === link.href 
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link
              href="/contact"
              className="bg-primary hover:bg-primary/90 text-primary-foreground px-5 py-2.5 rounded-full text-[13px] font-semibold transition-colors shadow-sm shadow-primary/20 whitespace-nowrap"
            >
              {t("nav.contactUs")}
            </Link>
            <LanguageToggle />
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div id="mobile-menu" className="md:hidden absolute top-full left-0 z-50 w-full bg-card border-b border-border p-5 flex flex-col gap-1 shadow-2xl animate-in slide-in-from-top-3">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
